...
export default function LivePreview({ data, className = '', activeSection = null }) {
  ...
  const fetchPreview = useCallback(async (previewData) => {
    ...
    try {
      ...
      if (html && html.includes('<')) {
        // ── previewOverride: array of strings joined, injected into <head> ──
          ...
          `<script>
            (function() {
              ...
              window.addEventListener('message', function(e) {
                ...
                if (!sec) return;
                (sectionMap[sec] || []).forEach(function(sel) {
                  try {
                ...
                if (lastHighlighted.length === 0 && val.length > 1) {
                  document.querySelectorAll('h1,h2,h3,h4,p,span,div,strong,em,b').forEach(function(el) {
        ...
        // ── taggerScript: injected before </body> — adds data-section to elements ──
        const taggerScript = `<script>
          (function() {
            function runTagger() {
              var d = window.__INVITE_DATA__ || {};
              // 1. data-i18n based (most precise)
              var di18nMap = {
              ...
              textFields.forEach(function(f) {
                f.keys.forEach(function(key) {
                  ...
                  candidates.forEach(function(el) {
                    ...
                    var cc = Array.from(el.children).filter(function(c){return c.tagName;}).length;
                    if (cc <= 1 && el.textContent.trim().includes(val)) {
  ...
  // Helper: send highlight to iframe (with delay to let tagger DOMContentLoaded run first)
  const sendHighlight = useCallback((section) => {
  ...
  // Track latest activeSection and send highlight
  useEffect(() => {
    activeSectionRef.current = activeSection;
    sendHighlight(activeSection);
  }, [activeSection, sendHighlight]);

  // Debounced preview re-fetch on data change
  useEffect(() => {
    ...
    <div className={`relative ${className}`}>
      ...
      {htmlContent ? (
        <iframe
          ...
          onLoad={() => {
            // Re-send highlight after iframe reloads
