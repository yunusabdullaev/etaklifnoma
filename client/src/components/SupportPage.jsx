...
export default function SupportPage({ token, onBack }) {
  ...
  const handleCreateTicket = async (e) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        setSubject(''); setNewMsg(''); setShowForm(false);
        setSuccess(t('support.sent'));
        setTimeout(() => setSuccess(''), 2000);
        await fetchTickets();
        if (data.data?.ticket?.id) await fetchMessages(data.data.ticket.id);
      }
    } catch (err) { console.error(err); }
  ...
  const statusLabels = {
    open: t('support.waiting'),
    answered: t('support.answered'),
              ...
              <motion.div key={msg.id || i}
                ...
                <div className={`max-w-[80%] rounded-2xl px-4 py-2.5 ${
                  ...
                  <p className={`text-[10px] mt-1 ${
                    ...
                    {msg.sender === 'admin' ? `🛟 ${t('support.admin')} · ` : ''}
        ...
        <form onSubmit={handleSendMessage} className="pt-3 border-t border-white/[0.06]">
          <div className="flex gap-2">
            <input type="text" value={newMsg} onChange={(e) => setNewMsg(e.target.value)}
              ...
              className="input-field flex-1 !rounded-full !py-2.5 !px-4 text-sm" autoFocus />
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-surface-400 text-sm mt-1">{t('support.subtitle')}</p>
          </div>
        </div>
      ...
      <AnimatePresence>
        {showForm && (
          <motion.form initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }}
            exit={{ opacity: 0, height: 0 }} onSubmit={handleCreateTicket}
            className="glass p-5 space-y-3 overflow-hidden">
            <h3 className="text-sm font-semibold text-white">{t('support.newTicket')}</h3>
            <div>
              <label className="text-xs text-surface-400 mb-1.5 block">{t('support.subject')} *</label>
              <input type="text" value={subject} onChange={(e) => setSubject(e.target.value)}
                placeholder={t('support.subjectPlaceholder')} className="input-field w-full" maxLength={200} />
            </div>
            <div>
              <label className="text-xs text-surface-400 mb-1.5 block">{t('support.message')} *</label>
              <textarea value={newMsg} onChange={(e) => setNewMsg(e.target.value)}
                placeholder={t('support.messagePlaceholder')}
                className="input-field w-full min-h-[100px] resize-y" maxLength={2000} />
            </div>
            <div className="flex justify-end gap-2">
              <button type="button" onClick={() => setShowForm(false)} className="btn-secondary text-sm">
                {t('support.cancel')}
              </button>
              <button type="submit" disabled={sending || !subject.trim() || !newMsg.trim()}
      ...
      {loading ? (
        ...
        <div className="glass p-12 text-center">
          ...
          <p className="text-surface-400 text-sm">{t('support.empty')}</p>
          <p className="text-surface-600 text-xs mt-1">{t('support.emptyDesc')}</p>
        </div>
      ) : (
          ...
          {tickets.map((ticket) => {
            ...
            return (
              <motion.button key={ticket.id}
                onClick={async () => { setChatLoading(true); await fetchMessages(ticket.id); setChatLoading(false); }}
