...
function buildContext(invitation, eventType, template) {
  ...
  // heroEmoji === 'none' → hideHeroEmoji=true (element is hidden in template)
  const rawHeroEmoji = customFields && customFields.heroEmoji ? customFields.heroEmoji : '';
  ctx['hideHeroEmoji'] = rawHeroEmoji === 'none';
  ctx['heroEmoji'] = (rawHeroEmoji && rawHeroEmoji !== 'none') ? rawHeroEmoji : '';
 ...
 * Always returns a non-empty string when locationName is provided (fallback to Yandex search).
 */
function getMapEmbedUrl(locationUrl, locationName) {
  ...
  try {
    ...
    // Cannot resolve redirect server-side → fallback to Yandex search by location name
  ...
  (function(){
    function tagSections() {
      ...
      var textFields = [
        ...
        { keys: ['eventTitle','eventTitleRu','eventTitleQq'], section: 'eventTitle' },
      ...
      // Waiting/footer message — data-i18n="waitingMsg" is most precise
      var waitEls = document.querySelectorAll('[data-i18n="waitingMsg"],.footer-msg,.waiting-msg,.waiting-message,.cd-bottom,.footer-waiting');
      waitEls.forEach(function(el){ if(!el.getAttribute('data-section')) el.setAttribute('data-section','waitingMsg'); });

      // Countdown & labels — data-i18n="countdownTitle"
  ...
  (function(){
    ...
    var translations = {
      ...
      qqCyr: {
        ...
        guestWelcome: 'Мехманлар күтип алыў',
        locationTitle: 'Локация',
        viewMap: 'Картада көриў',
        programTitle: 'Кеше программасы',
        dressCode: 'Дресс код',
        waitingMsg: 'Сизди күтип қаламыз!',
        galleryTitle: 'Фото көргизбе',
        wishesTitle: '💌 Тилек ҳәм қутлықлаў',
        wishesSubtitle: 'Тилек ҳәм қутлықлаўларыңызды қалдырың',
        wishesName: 'Атыңыз',
        wishesMessage: 'Тилеклериңиз...',
        wishesSend: 'Жибериў',
        wishesSent: 'Тилеклериңиз жиберилди! Рахмет!',
        wishesError: 'Қателик жүз берди.',
        envOpen: 'Ашыў ушын басың',
        shareWa: 'WhatsApp тармағында үлесиў',
        shareTg: 'Telegram тармағында үлесиў',
        calBtn: 'Календарыма қосыў',
        printBtn: 'PDF қылып сақлаў',
        inviteText: '💍 Сизди таклифнамамызға шақырамыз!',
        ...
        bdAge: "жас",
        gradYear: "питириўшилер",
        jubYears: "жыллық",

        gradEventLabel: 'Питкериўшилер кешеси',
        gradCountdownTitle: 'Ушырасыў кешесине қалған ўақыт:',
        gradDetailsTitle: 'Ушырасыў тафсилатлары',
        gradProgramTitle: 'Кеше программасы',
        gradWaitingMsg: 'Сизди күтип қаламыз! 🎓',

        jubEventLabel: 'Юбилей шақырыўы',
        jubCountdownTitle: 'Байрамға шекем қалған ўақыт',
